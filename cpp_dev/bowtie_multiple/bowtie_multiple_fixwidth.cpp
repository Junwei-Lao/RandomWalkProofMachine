...
double point_list[] = {1642, 1808, 2031, 2192, 2425, 2608, 2778, 3026, 3241, 3481, 3685, 3955, 4145, 4425, 4646, 4845, 5140, 5402, 5710, 5922, 6240, 6491, 6712, 7045, 7308, 7615, 7925, 8280, 8521, 8805, 9175, 9425, 9805, 10106, 10498, 10808, 11211, 11480, 11802, 12220, 12498, 12926, 13265, 13705, 14053, 14405, 14860, 15161, 15626, 15995, 16408, 16785, 17232, 17658, 18048, 18551, 18880, 19393, 19800, 20251, 20666, 21161, 21625, 22053, 22603, 22961, 23521, 23966, 24333, 24908, 25450, 25952, 26418, 27016, 27491, 27880, 28493, 28980, 29605, 30101, 30736, 31145, 31790, 32303, 32721, 33381, 33906, 34578, 35112, 35795, 36232, 36778, 37476, 37922, 38630, 39193, 39913, 40485, 41061, 41681, 42265, 43010, 43603, 44240, 44841, 45730, 46218, 46832, 47615, 48112, 48905, 49536, 50211, 50850, 51625, 52313, 52965, 53795, 54321, 55161, 55830, 56365, 57360, 58042, 58768, 59458, 60336, 61035, 61778, 62485, 63196, 64101, 64821, 65736, 66313, 67238, 67975, 68561, 69501, 70250, 71202, 71960, 72923, 73528, 74298, 75276, 75890, 76878, 77665, 78665, 79461, 80471, 81105, 81913, 82938, 83755, 84616, 85618, 86666, 87322, 88160, 89223, 89888, 90961, 91816, 92715, 93765, 94633, 95545, 96421, 97531, 98225, 99345, 100238, 101175, 102272, 103178, 104128, 105042, 106200, 106922, 108090, 109021, 109956, 111141, 112085, 113280, 114025, 115230, 116191, 116945, 118165, 119138, 120370, 121352, 122595, 123368, 124621, 125620, 126402, 127670, 128905, 129961, 130981, 132271, 133073, 134105, 135410, 136221, 137536, 138818, 140146, 140970, 142032, 143375, 144208, 145561, 146640, 147763, 149093, 150468, 151321, 152421, 153811, 154673, 156073, 157190, 158351, 159728, 160858, 162032, 163170, 164608, 165498, 166946, 168101, 169260, 170725, 171893, 173368, 174281, 175766, 176951, 178180, 179373, 180840, 182082, 183288, 184811, 185752, 187285, 188508, 189458, 191006, 192521, 193801, 195045, 196615, 197585, 198841, 200426, 201405, 203000, 204562, 206170, 207162, 208780, 210071, 211072, 212705, 214008, 215355, 216965, 218620, 219641, 220965, 222635, 223665, 225345, 226686, 228071, 229728, 231082, 232480, 233842, 235560, 236618, 238346, 239725, 241465};
...
int main() {
    ...
    for (int element = 0; element < 300; element++) {
        ...
        int gate1StopLeftCount = 0;
        int gate1StopRightCount = 0;
        int gate2StopLeftCount = 0;
        int gate2StopRightCount = 0;
        int gate3StopLeftCount = 0;
        int gate3StopRightCount = 0;
        ...
        for (int overall_count = 0; overall_count < sample_size; overall_count++) {
            ...
            */
            ...
            if (leftTrapozoidPoints > rightTrapozoidPoints) {
                ...
                gate3StopLeft[gateCrossFromLeft] = uniquePointGate3Right;
                gateCrossFromLeft += 1;

                if (uniquePointGate1Right != 0) {
                    gate1StopLeft[gate1StopLeftCount++] = uniquePointGate1Right;   //unique point is actually the total walk time
                }
                if (uniquePointGate2Right != 0) {
                    gate2StopLeft[gate2StopLeftCount++] = uniquePointGate2Right; //StopLeft means start from left trapozoid and end at gate right
                }
                if (uniquePointGate3Right != 0) {
                    gate3StopLeft[gate3StopLeftCount++] = uniquePointGate3Right;
                }
                ...
                if (uniquePointGate1Left != 0) {
                    gate1StopRight[gate1StopRightCount++] = uniquePointGate1Left;   //unique point is actually the total walk time
                }
                if (uniquePointGate2Left != 0) {
                    gate2StopRight[gate2StopRightCount++] = uniquePointGate2Left; //StopRight means start from right trapozoid and end at gate left
                }
                if (uniquePointGate3Left != 0) {
                    gate3StopRight[gate3StopRightCount++] = uniquePointGate3Left;
                }
            }
        ...
        for (int i = 0; i < gate1StopLeftCount; i++) {
            sumGate1StopLeft += gate1StopLeft[i];
        }
        for (int i = 0; i < gate1StopRightCount; i++) {
            sumGate1StopRight += gate1StopRight[i];
        }
        for (int i = 0; i < gate2StopLeftCount; i++) {
            sumGate2StopLeft += gate2StopLeft[i];
        }
        for (int i = 0; i < gate2StopRightCount; i++) {
            sumGate2StopRight += gate2StopRight[i];
        }
        for (int i = 0; i < gate3StopLeftCount; i++) {
            sumGate3StopLeft += gate3StopLeft[i];
        }
        for (int i = 0; i < gate3StopRightCount; i++) {
        ...
        */
        ...
        // i-th time, total point, Average walk time, gate1 count (Left), average LTOF gate1 (left), gate2 count (Left), average LTOF gate2 (left), gate3 count (Left), average LTOF gate3 (left), gate1 count (Right), average LTOF gate1 (right), gate2 count (Right), average LTOF gate2 (right), gate3 count (Right), average LTOF gate3 (right)
        std::cout << "\r" << element << ',' << point_list[element] << ',' << std::fixed << std::setprecision(6) << sumMean / sample_size << ',' <<sumGateLeftMean / gateCrossFromLeft << ',' << sumGate1StopLeft / gate1StopLeftCount << ',' << sumGateLeftMean2 / gateCrossFromLeft << ',' << sumGate2StopLeft / gate2StopLeftCount << ',' << sumGateLeftMean3 / gateCrossFromLeft << ',' << sumGate3StopLeft / gate3StopLeftCount << ',' << sumGateRightMean / gateCrossFromRight << ',' << sumGate1StopRight / gate1StopRightCount << ',' << sumGateRightMean2 / gateCrossFromRight << ',' << sumGate2StopRight / gate2StopRightCount << ','  << sumGateRightMean3 / gateCrossFromRight << ',' << sumGate3StopRight / gate3StopRightCount << std::flush;
        std::cout << std::endl;
    }
