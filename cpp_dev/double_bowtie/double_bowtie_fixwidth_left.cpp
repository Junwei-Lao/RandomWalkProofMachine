...
int main() {
    ...
    for (int element = 0; element < 300-38; element++) {
        ...
        for (int overall_count = 0; overall_count < sample_size; overall_count++) {
            ...
            if (isLeft && gateCrossedCount != 0 && gateCrossedCount2 != 0 && gateCrossedCount3 != 0) {
                ...
                gate1StopLeft.push_back(finalCrossGate1);
                gate2StopLeft.push_back(finalCrossGate2);
                gate3StopLeft.push_back(finalCrossGate3);
        ...
        // i-th time, total point, Average walk time, gate1 count (Left), average LTOF gate1 (left), gate2 count (Left), average LTOF gate2 (left), gate3 count (Left), average LTOF gate3 (left)
        std::cout << "\r" << element << ',' << point_list[element] << ',' << std::fixed << std::setprecision(6) << sumMean / sample_size << ',' <<sumGateLeftMean / gateValueListLeft.size() << ',' << sumGate1StopLeft / gate1StopLeft.size() << ',' << sumGateLeftMean2 / gateValueListLeft.size() << ',' << sumGate2StopLeft / gate2StopLeft.size() << ',' << sumGateLeftMean3 / gateValueListLeft.size() << ',' << sumGate3StopLeft / gate3StopLeft.size() <<  std::flush;
        std::cout << std::endl;
    }
