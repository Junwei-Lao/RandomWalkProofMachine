...
def mainFunc(l, totalpoint):
    ...
    machine, walk_count, path_ = initialize(hypo_side, length)
    ...
    with open(txt_path, "w") as f:
        ...
        while (True):
            machine, walk_count, path_, unique_visit = RandomWalk(machine, walk_count, path_, hypo_side, length, unique_visit)
            f.write(f"{machine[0]} {machine[1]} {len(path_)} {len(unique_visit)}\n")
